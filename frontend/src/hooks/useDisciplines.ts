  ...
  return useQuery<Discipline[], Error>({
    ...
    queryFn: async () => {
      ...
      if (response.success) {
        ...
        throw new Error(response.message || 'Failed to fetch disciplines');
      }
    },
...
export const useAllDisciplines = () => {
  return useQuery<Discipline[], Error>({
    ...
    queryFn: async () => {
      const response = await disciplineAPI.getAllIncludingInactive();
      
      if (response.success) {
        return response.data;
      } else {
        throw new Error(response.message || 'Failed to fetch all disciplines');
      }
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000, // 10 minutes (formerly cacheTime in v4)
